$ ->
  ...
  class BracketView
    constructor: (options) ->
      {@moderatorStatus, @bracket, @matchHeight, @matchPadding, @matchWidth, @matchBorder}

    renderBracket = ->
      @output = ""
      $.each @bracket, (round_index, round) ->
        if round_index == bracket.length-1
        ...
        $.each round, (match_index, match) ->
          ...
          @determineHeader(round_index, round)
          if match != null
            $.each match, (user_index, user) ->
              if @moderatorStatus is "true"
              ...
              else if @moderatorStatus is "false"
    ...
    determineHeader = (round_index, round, match_index)->
      if round_index == @bracket.length - 1
        ...
        @output += "<div> Ro#{round.length*2} Match #{match_index+1} </div>"
    ...
    setStyles: ->
      ...
      $.each @bracket, (round_index, round) ->
        ...
        if round_index > 1 && round_index != @bracket.length - 1
