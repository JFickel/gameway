class Invitation < ActiveRecord::Base
  ...
  validates :user_is_not_in_team, :user_is_not_tournament_owner, :user_is_not_broadcaster_already, :user_is_not_moderator_already
  ...
  def set_message
    ...
    if team_id.present?
      message ="#{sender.username} has sent you a team invitation to join #{Team.find(invitation_params[:team_id]).name}."
    elsif tournament_id.present? && role == 'moderator'
      "#{sender.username} has invited you to moderate #{Tournament.find(invitation_params[:tournament_id]).title}"
    elsif tournament_id.present? && role == 'broadcaster'
      "#{sender.username} has invited you to broadcast #{Tournament.find(invitation_params[:tournament_id]).title}"
    end
  end
  ...
  def user_is_not_tournament_owner
    tournament = Tournament.find(invitation_params[:tournament_id])
    user = User.find(invitation_params[:user_id])
